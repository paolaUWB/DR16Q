...
highZlbol = np.array(highZfile["bol_lum"])
highZmassErr = np.vstack((highZfile["bhm_err_minus"], highZfile["bhm_err_plus"]))
highZeddErr  = np.vstack((highZfile["edd_ratio_err_minus"], highZfile["edd_ratio_err_plus"] ))

print("Error shape: " + str(highZmassErr.shape))

highZeddErr[0] = find_log_err(highZedd, highZeddErr[0], False)
highZeddErr[1] = find_log_err(highZedd, highZeddErr[1], True)
highZmassErr[0] = find_log_err(highZmass, highZmassErr[0], False)
highZmassErr[1] = find_log_err(highZmass, highZmassErr[1], True)

highZmass = np.log10(highZmass)
highZedd = np.log10(highZedd)
highZlbol = np.log10(highZlbol)
...
def scatter_hist2(x, y, ax, ax_histx, ax_histy, color, area, mult, factor, ax_set, limx, limy,binwidth_x,binwidth_y, marker="."):
    # no labels
    ...
    ax.scatter(x, y, s = area, color = color, marker = marker)
    ax.text(10.5,0.5,'')
...
ax.errorbar(highZmass, highZedd, xerr=highZmassErr, yerr=highZeddErr, fmt='none', ecolor='red')
...
scatter_hist2(highZmass, highZedd, ax ,ax_histx, ax_histy , 'red', 60, 'yes', 15, ax_set_2, limx=masslim, limy=reddlim, binwidth_x = op_binmbh_knuthp, binwidth_y = op_binedd_knuthbal, marker = "*" )
...
scatter_hist2(xem, yer, ax, ax_histx, ax_histy ,'purple', 60, 'yes', 10, ax_set_2,limx=masslim, limy=reddlim,binwidth_x = op_binmbh_knuthp, binwidth_y = op_binedd_knuthbal)

# add label for error bars in legend
rankineErrorBarLeg = ax.errorbar(0, 0, yerr=1.0, fmt='none', label='Rankine')

# ax.add_artist(ax.legend(title='test'))
ax.legend(['Rankine+2020 BALs vmin<10,000 km/s','Rankine+2020 BALs 10,000 km/s<vmin<25,000 km/s','QSO z > 7.0','RH+(in prep) EHVO', rankineErrorBarLeg ],loc='lower left')
...
scatter_hist2(highZlbol, highZedd, ax ,ax_histx, ax_histy , 'red', 60, 'yes', 15, ax_set_2, limx=lbollim, limy=reddlim, binwidth_x = op_binlbol_knuthbal, binwidth_y = op_binedd_knuthbal, marker="*" )
...
scatter_hist2(xel, yer, ax, ax_histx, ax_histy ,'purple', 60, 'yes', 10, ax_set_2,limx=lbollim, limy=reddlim, binwidth_x = op_binlbol_knuthbal, binwidth_y = op_binedd_knuthbal)


# ax.add_artist(ax.legend(title='test'))
ax.legend(['Rankine+2020 BALs vmin<10,000 km/s','Rankine+2020 BALs 10,000 km/s<vmin<25,000 km/s','High Redshift QSO', 'RH+(in prep) EHVO'],loc='lower left')
# ax.legend(['Rankine+2020 Parent sample','RH+(in prep) EHVO'],loc='lower left')
ax.set_title("Title for second plot")
# plt.savefig(PLOT_DIREC + 'Redd_Lbol.png') 
plt.show()
