...
CONFIG_FILE = sys.argv[1] if len(sys.argv) > 1 else "DR" + DR + "_EHVO_updated.csv" #"_sorted_norm.csv"
#CONFIG_FILE = "mostlum_sorted_norm.csv"
...
STARTS_FROM, ENDS_AT = 61, 71 #11521, 11521  ## [1-10, 899-1527 for dr9] [1-21823 [21851 for dr16 (21852-21859 are high redshift cases - must set dynamic = yes to run)] 
...
for spectra_index in range(STARTS_FROM, ENDS_AT + 1):
    ...
    if dynamic == 'yes':
        ...
        wavelength_observed_from = WAVELENGTH_RESTFRAME.start * (1 + z) 
        wavelength_observed_to = WAVELENGTH_RESTFRAME.end * (1 + z) 
        ...
        print("Anchor Points [OLD]: LEFT: " + str(anchor_point[0][0]) + ", MIDDLE: " + str(anchor_point[1][0]) + ", RIGHT: " + str(anchor_point[2][0]))
        ...
        while try_again == 'no':
            ...
            while range_value > 10:
                range_value = int(input("Please choose a range less than 10: "))
            ...
            draw_dynamic_points(spectra_index, wavelength, wavelength_observed_from, wavelength_observed_to, flux, test1, test2, number_of_anchor_points, anchor_pts, max_peak, bf, cf, z, snr, snr_mean_in_ehvo, current_spectrum_file_name, 'null', error) 
        ...
        if try_again == 'yes':
            draw_dynamic_points(spectra_index, wavelength, wavelength_observed_from, wavelength_observed_to, flux, test1, test2, number_of_anchor_points, anchor_pts, max_peak, bf, cf, z, snr, snr_mean_in_ehvo, current_spectrum_file_name, GOOD_FIT_PDF, error)
            if save_new_output_file == 'yes':   
    ...
    ################################### END DYNAMIC PLOTTING ####################################
        ...
        wavelength_observed_from = np.min(wavelength) # (z + 1) * WAVELENGTH_RESTFRAME.start
        wavelength_observed_to = np.max(wavelength) #(z + 1) * WAVELENGTH_RESTFRAME.end
