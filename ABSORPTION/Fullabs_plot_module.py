...
def draw_abs_figure(spectra_count_abs, spectra_index, velocity, flux_normalized, error, savefile_name, spectra_name, redshift, snr, max_peak, limits_percent):
    ...
    snr = round(snr, 2)
    min_peak = -0.1
    plt.title(str(spectra_count_abs) + ' abs |' + str(spectra_index) + ' tot: ' + str(spectra_name) + ', z=' + str(redshift) + ' snr=' + str(snr), loc = 'center', wrap = True, fontsize = 15)
    plt.axhline(y = limits_percent, color='r', linestyle = '--')    
    plt.axhline(y = 1.0)
