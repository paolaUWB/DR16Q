...
from Fullabs_function_moduleVCentCalc import smooth, abs_parameters_plot_optional
from Fullabs_plot_module import draw_abs_figure
...
ABSORPTION_OUTPUT_PLOT_PDF = PdfPages(OUT_DIREC + '1-108BI' + str(BALNICITY_INDEX_LIMIT) + '.pdf') 

ABSORPTION_TABLE = OUT_DIREC + '1-108BI2000.csv'
...
if (want_csv == 'yes'):
    field = ['NORM SPECTRA FILE NAME','BI TOTAL','BI INDIVIDUAL','VMINS', 'VMAXS', 'EW INDIVIDUAL', 'MAX DEPTH', 'AVERAGE DEPTH', 'VCENTRAL']
...
for spectra_index in range(STARTS_FROM, ENDS_AT + 1):
    ...
    BI_total, BI_individual, BI_all, vmins, vmaxs, EW_individual, final_depth_individual, final_depth_all_individual, beta, vminindex_for_range, vmaxindex_for_range, v_cent, avg_depth = abs_parameters_plot_optional(
        z, wavelength, normalized_flux, BALNICITY_INDEX_LIMIT, VELOCITY_LIMIT, trough_detection_percent, limits_percent)

    max_peak = np.max(normalized_flux[vmaxindex_for_range + 1 : vminindex_for_range + 1])

    ############################# putting things into a text file or plot #######################################
    fields = [norm_spectrum_file_name, BI_total, BI_individual, vmins, vmaxs, EW_individual, final_depth_individual, avg_depth, v_cent]
    ...
    if (all_plot_and_text == 'yes'): # plot all is yes, graph everything but only text file for when abs is found
        ...
        if (len(vmaxs) != 0): # text file created only when absorption is found
            ...
            text = [f"{str(abs_count)} abs | {str(all_count)} tot",
                    ...
                    f"Max Depth: {final_depth_individual}"
            ...
            abs = 'no'
        draw_abs_figure(
            abs, all_count, beta, normalized_flux, normalized_error, ABSORPTION_OUTPUT_PLOT_PDF, norm_spectrum_file_name, z, calc_snr, max_peak, limits_percent)
        ...
        if (len(vmaxs) != 0):
            ...
            abs = abs_count
            draw_abs_figure(
                abs_count, all_count, beta, normalized_flux, normalized_error, ABSORPTION_OUTPUT_PLOT_PDF, norm_spectrum_file_name, z, calc_snr, max_peak, limits_percent)
