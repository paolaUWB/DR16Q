...
with PdfPages(output_morphed_pdf) as pdf:
    for spectra_index in range(STARTS_FROM, ENDS_AT + 1):
        ...
        print("check " + str(spectra_index) + ', ' + str(norm_spectrum_file_name))
        pdf.savefig(fig)
        plt.show(fig)
        plt.close(fig)
    ...
    '''
    we want to compare, original, reconstruction and error in terms of velocity
    ...
    whichever one was chosen, have it produce into a pdf, we found a function in absorption.py; check line 14
    '''
