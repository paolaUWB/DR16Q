...
OUTDIREC = os.getcwd() + '/../CROSS_CORRELATION/CIV_Emission_Absorption/OUTPUT_FILES/'


above_below_partition = 'above'
above_below_partition = 'below'


if above_below_partition=='above':
    num=21
elif above_below_partition=='below':
    num=61
...
for i in range(num):
    ...
    if use_yaml==True:
        ...
        with open(OUTDIREC + 'spec_'+ str(above_below_partition) +'_'+str(i)+'.yaml') as file:
            read_params = yaml.safe_load(file)
    
        #assert read_params == parameters
    
            
        save_file_name = read_params['save_file_name']
        norm_spectra = read_params['norm_spectra']
        
        zem = read_params['zem']
        
        vmin = [read_params['vmin']]
        vmax = [read_params['vmax']]
        
    else:    
        
        specdirec = os.getcwd() + '/../DR16Q_EHVO/NORM_DR16Q_EHVO/'
        
        save_file_name = 'spec-6182' 
        norm_spectra = 'spec-6182-56190-0657norm.dr16'
        
        zem = 2.96478 # redshift of norm_spectra
        
        vmin = [-33434.85851] # make smaller to move right line right (bigger to move right line left)
        vmax = [-35682.5159] # make bigger to move left line left (smaller to move left line right)
    ...
    if use_yaml==True:
        try:
            specdirec = os.getcwd() + '/../DR16Q_EHVO/NORM_DR16Q_EHVO/'
            data = np.loadtxt(specdirec+norm_spectra+'16') 
        except:
            specdirec = os.getcwd() + '/../DR9Q_EHVO/NORM_DR9Q_EHVO/'
            data = np.loadtxt(specdirec+norm_spectra+'9') 
    else:
    ...
    plt.savefig(OUTDIREC + pp2, dpi=100)
