...
df = pd.read_csv('FULLCorrected Variability Results - 1.0EWUnsmoothedFiltered.csv')
...
if 'OG EHVO Obs' in df.columns and 'SDSS name' in df.columns:
    ...
    for name in df['SDSS name'].unique():
        ...
        if name == 'J144911.37+370734.0': print(values)
...
new_df['Difference'] = new_df['Difference']
