...
save_ehvo_txt_files = 'yes' # do you want to save new ehvo output files? 'yes'/'no'
...
OUT_DIREC = os.getcwd() + '/VARIABILITY/DATA_VARIABILITY/'
...
if DR == '9':
    ...
    zem, snr, EHVO_spectra_names = read_file(EHVO_FILE)
...
##------ output files
SPEC_NAMES_FILE = os.getcwd() + '/VARIABILITY/dr' + DR + '_spec_names.txt'
...
#%%
##------ read files in variability directory (duplicate files)
...
#%%
##------ find duplicates & define variables
spec_names = []
spec_duplicates = []
spec_duplicates_index = []

EHVO_duplicates_idx = []
EHVO_duplicates_names = []
EHVO_duplicates = []

foldernames = []


for i in range(len(fits_PLATE)): # 750414 total spectra in DR16 fits table
    duplicates = []
    
    spec_names.append('spec-' + fits_PLATE[i].zfill(4) + '-' + fits_MJD[i] + '-' + fits_FIBER[i].zfill(4))
    foldernames.append('J' + fits_SDSS_NAME[i])
    
    if save_txt_files == 'yes':
        print_to_file(spec_names[i], SPEC_NAMES_FILE)
    ...
    if duplicates != []:
        spec_duplicates.append(duplicates)
    
    else: 
        spec_duplicates.append('-1')
    
    if duplicates != []:
        for k in range(len(EHVO_spectra_names)):
            if DR == '16':
                EHVO_spec = EHVO_spectra_names[k][:-11]
            if DR == '9': 
                EHVO_spec = EHVO_spectra_names[k][:-4]
            
            for l in range(len(duplicates)):
                if duplicates[l] == EHVO_spec:
                    EHVO_duplicates_idx.append(m)
                    EHVO_duplicates_names.append(spec_names[m])
                    EHVO_duplicates.append(spec_duplicates)
