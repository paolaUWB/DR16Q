...
quasardirec = 'J162122.54+075808.4/' # Name of the object you are observing (same name as the directory folder it's contained in). If you don't have these directory names,
...
zem =2.138# redshift of the quasar
coem=zem+1.
...
for i in range(len(norm_list)):
    ...
    if error_diagnostics == True:
        ...
        print('Norm error max: ' + str(np.max(error)))
...
for i in range(len(dered_list)):
    ...
    if error_diagnostics == True:
        ...
        print('Dered error max: ' + str(np.max(error)))
