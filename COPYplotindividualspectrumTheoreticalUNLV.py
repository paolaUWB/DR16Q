...
save_file_name = 'spec004_UNLV' 
norm_spectra = 'COPY1_1e9_b6985_c3rad_l-2g1al075mw95_wa200_1_5ed_2_sorted89.dat'
...
wavelength_emit1_initial = 800.  # left xlim in restframe
wavelength_emit2_initial = 2000.  # right xlim in restframe

vmin = [-4000, -48000] # make smaller to move right line right (bigger to move right line left)
vmax = [-14000, -58000] # make bigger to move left line left (smaller to move left line right)
...
#1600+ Angstroms, Verner Table Doublets:
Al3llred = 1862.7900   #Aluminum 3
Al3llblue = 1854.7160
...
for k in range(0,len(vmin)):
    ...
    if CIVabs == 'yes':
        ...
        plt.text(CIVll*(1.+zabs_min[k])-30.,0.135-0.1*k,'CIV',color=color[0],fontname='serif',weight='bold')

    if SiIVabs == 'yes':
        plt.axvspan(SiIVllblue*(1.+zabs_max[k]),SiIVllred*(1.+zabs_min[k]), alpha=0.2, color=color[3])
        plt.text(SiIVll*(1.+zabs_min[k])-30.,0.135-0.1*k,'SiIV',color=color[3],fontname='serif',weight='bold')
    
    if NVabs == 'yes':
        plt.axvspan(NVllblue*(1.+zabs_max[k]),NVllred*(1.+zabs_min[k]), alpha=0.2, color=color[1])
        plt.text(NVll*(1.+zabs_min[k]),0.18-0.1*k,'NV',color='xkcd:azure',fontname='serif',weight='bold')
    
    if OVIabs == 'yes':
        plt.axvspan(OVIllblue*(1.+zabs_max[k]),OVIllred*(1.+zabs_min[k]), alpha=0.2, color=color[2])
        plt.text(OVIll*(1.+zabs_min[k]),0.14-0.1*k,'OVI',color=color[2],fontname='serif',weight='bold')

    if AlIIIabs == 'yes':
        plt.axvspan(Al3llblue*(1.+zabs_max[k]),Al3llred*(1.+zabs_min[k]), alpha=0.2, color=color[1])
        plt.text(AlIII*(1.+zabs_min[k])-20.,0.145-0.1*k,'AlIII',color=color[4],fontname='serif',weight='bold')
...
# AlIII
    if Al3llblueabs == 'yes' and Al3llredabs == 'yes' :
        ...
        plt.axvspan(Al3llred*(1.+zabs_max[k]),Al3llblue*(1.+zabs_min[k]), alpha=0.2, color=color[1])
        plt.text(Al3llblue*(1.+zabs_min[k]),0.19-0.1*k,'AlIII',color=color[4],fontname='serif',weight='bold')
