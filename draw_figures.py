...
def draw_original_figure(figure_index: int, original_ranges: RangesData, data: FigureDataOriginal, test1: RangesData, test2: RangesData, wavelength_observed_from, wavelength_observed_to, max_peak, FILE, flags, anchor_pts, z):
    ...
    # plt.axvspan(1420 * (z + 1), 1430 * (z + 1), 0, 1, alpha = 0.5, color = "mediumslateblue")
    # ## MIDDLE POINT NEW
...
def draw_normalized_figure(figure_index: int, original_ranges: RangesData, figure_data: FigureData, flux_normalized, error_normalized,
                            test1: RangesData, test2: RangesData, normalized_flux_test_1, normalized_flux_test_2, wavelength_observed_from, wavelength_observed_to, max_peak, FILE, min_flux_green_region, min_flux_pink_region, max_flux_green_region, max_flux_pink_region, z):
    ...
    plt.plot(test2.wavelength, normalized_flux_test_2, color = test_2_color, zorder = 1, linestyle = "-")
    ...
    ## GREEN
    # plt.axhspan(np.median(normalized_flux_test_1) - 0.05, np.median(normalized_flux_test_1) + 0.05, xmin = 0, xmax = 1, zorder = 1, alpha = 0.5, color = test_1_color)
    # plt.hlines(np.median(normalized_flux_test_1) + 0.05, wavelength_observed_from, wavelength_observed_to, zorder=2, color='black')
    # plt.hlines(np.median(normalized_flux_test_1) - 0.05, wavelength_observed_from, wavelength_observed_to, zorder=2, color='black')
    # plt.hlines(np.median(normalized_flux_test_1), wavelength_observed_from, wavelength_observed_to, zorder = 2, color = 'black', linestyle = 'dashed')
    # plt.axvline(1315 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    # plt.axvline(1325 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    ## PINK
    # plt.axhspan(np.median(normalized_flux_test_2) - 0.05, np.median(normalized_flux_test_2) + 0.05, xmin = 0, xmax = 1, zorder = 1, alpha = 0.2, color = test_2_color)
    # plt.hlines(np.median(normalized_flux_test_2) + 0.05, wavelength_observed_from, wavelength_observed_to, zorder=2, color='black')
    # plt.hlines(np.median(normalized_flux_test_2) - 0.05, wavelength_observed_from, wavelength_observed_to, zorder=2, color='black')
    # plt.hlines(np.median(normalized_flux_test_2), wavelength_observed_from, wavelength_observed_to, color = 'black', linestyle = 'dashed')
    # plt.axvline(1350 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    # plt.axvline(1360 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    
    ### TO PLOT RANGE ALLOWED FOR GOOD FIT (TEST#3)
    # ## GREEN
    # plt.hlines(min_flux_green_region, np.min(wavelength_observed_from), np.max(wavelength_observed_to), zorder=2, color='black')
    # plt.hlines(max_flux_green_region, np.min(wavelength_observed_from), np.max(wavelength_observed_to), zorder=2, color='black')
    # plt.axvline(1315 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    # plt.axvline(1325 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    # plt.axhspan(min_flux_green_region, max_flux_green_region, xmin = 0, xmax = 1, zorder = 1, alpha = 0.5, color = test_1_color)
    # plt.hlines(np.median(normalized_flux_test_1), wavelength_observed_from, wavelength_observed_to, zorder = 2, color = 'red', linestyle = 'dashed', linewidth = 0.7)
    ...
    # plt.axvline(1350 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    # plt.axvline(1360 * (z + 1), 0, 1, color = "black", linestyle = "dotted")
    # plt.axhspan(min_flux_pink_region, max_flux_pink_region, xmin = 0, xmax = 1, zorder = 1, alpha = 0.2, color = test_2_color)
    # plt.hlines(np.median(normalized_flux_test_2), wavelength_observed_from, wavelength_observed_to, zorder = 2, color = 'red', linestyle = 'dashed', linewidth = 0.7)
    ...
    plt.ylim(0, max_peak + (max_peak / 1.5))
    FILE.savefig(bbox_inches='tight')
    plt.close(figure_index)
