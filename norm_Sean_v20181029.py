...
def normalize_spectra(starting_index: int, ending_index: int):
    good_categorized_spectra_range = range(starting_index, ending_index)
    ...
    ###########THIS HUGE FOR LOOP STARTS HERE AND FINISHES AROUND LINE ~230 ########
    for index in good_categorized_spectra_range:
        ...
        # REQUIREMENT FOR USING #POWERLAW. CHANGE THIS CHECK!
        if (bf)*(np.power(median_wavelength_point_B, cf)) > (median_flux_point_B) - (3)*(median_error_point_B):
    ...
    ############### THE FOR LOOP STARTED AT LINE ~130 FINISHED HERE #####################
