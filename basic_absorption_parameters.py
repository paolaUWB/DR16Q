...
def find_absorption_parameters(wavelength, normalized_flux, redshift, BALNICITY_INDEX_LIMIT = 1000, VELOCITY_LIMIT = Range(-30000, -60000.)):
    """
    Parameters
    ----------
    wavelength
    converted to velocity using redshift

    normalized_flux
    the flux associated with each wavelength

    redshift
    Also commonly labeled as "z;" allows for wavelength to convert to velocities
    ...
    beta = wavelength_to_velocity(redshift, wavelength)
...
#    vmaxs = np.array(vmaxs)
    ...
    return BI_total, BI_individual, vmins, vmaxs, EW_individual, final_depth_individual
